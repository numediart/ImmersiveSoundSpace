...
### OSC messages format
If you plan to create your own listener, here is the definition of the OSC messages sent by the python script. There is a new message for each tracker, at each frame. That means, if your setup uses 3 trackers and `--framerate` option is set to 30, you will actually have to manage 90 OSC messages per second. **The UnityOSC library has issues with OSC messages comming faster than the display refresh rate, queuing them indefinitely, which might lead to incresing latency.**
